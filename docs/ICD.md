...
### 2.3 Prevención de Ecos de Red

DDM debe distinguir entre dos tipos de actualizaciones de su estado interno:

- **Origen local:** El operador o un sensor local generó el evento. DDM debe notificar a ReplicationEngine para que lo distribuya.
- **Origen remoto:** ReplicationEngine recibió el objeto desde la red y lo está inyectando en DDM. DDM **no debe** re-notificar a ReplicationEngine. Hacerlo generaría un bucle de retroalimentación infinita.

ReplicationBridge es responsable de proveer el mecanismo que permite a DDM distinguir estos dos flujos.
...
|--------|-------|-------------|
| `NetworkStatusChanged` | `status_code: int` | Cambio en el estado de la red. Valores sugeridos: `0=Desconectado`, `1=Conectado`, `2=Sincronizando`. |
