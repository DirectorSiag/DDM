...
public:
    ...
    QString usage() const override {
        return
            "add <--type <SPC|LINCO|ASW|OPS|HECO|APC|AAW|EW>|-t <...>|-f|-e|-u> [identidad] <x> <y> [legacyVelKnots] [legacyCourseDeg]\n"
            "\n"
            "Tipo táctico / ambiente (obligatorio):\n"
            "  --type <SPC|LINCO|ASW|OPS|HECO|APC|AAW|EW>   (alias: -t)\n"
            ...
            "  --spd <DM/h>          (0..99.9)\n"
            "  --kt <knots>          (>=0; alternativa a --spd, en nudos)\n"
            "  --crs <deg>           (0..359)\n"
            "  --fc <1..6>\n"
            "  --asgc <texto>\n"
            "  --linky <R|C|T|S>\n"
