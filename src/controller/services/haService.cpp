...
void HaService::initSession(double xDm, double yDm) {
    ...
    m_timers[idx].start();
    m_ctx->haSessions[idx].fallTimeLocal = m_timers[idx].fallTimeLocal();
    m_ctx->haSessions[idx].fallTimeUtc   = m_timers[idx].fallTimeUtc();
...
void HaService::update() {
    ...
    for (int i = 0; i < CommandContext::kMaxHaSessions; ++i) {
        ...
        m_ctx->haSessions[i].elapsedTime = m_timers[i].elapsedTime();
    }
}
